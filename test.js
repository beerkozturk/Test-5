...
export default function AdminHome({ userData }) {
  ...
  const getPaginatedData = () => {
    fetch("http://localhost:8850/users?page=1&limit=5", {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const getAllUser = () => {
    fetch("http://localhost:8850/getAllUser", {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const deleteUser = (id, name) => {
    if (window.confirm(`Are you sure you want to delete ${name}`)) {
      fetch("http://localhost:8850/deleteUser", {
        method: "POST",
        crossDomain: true,
        headers: {
...
/> */}
  ...
  function handlePageClick(e) {
    ...
    fetch(`http://localhost:8850/users?page=${movies.current}&limit=${limit}`, {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => {
